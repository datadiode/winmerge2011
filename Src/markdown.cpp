 ...
 *			Recommended reading:
 *
 *			www.javaworld.com/javaworld/jw-02-2002/jw-0208-xmljava.html (SAX2 Basics)
 *			www.javaworld.com/javaworld/jw-03-2002/jw-0329-xmljava2.html (SAX vs Pull)
 *			www.javaworld.com/javaworld/jw-04-2002/jw-0426-xmljava3.html (Performance)
...
HString *CMarkdown::_HSTR::Uni(const EntityMap &map, unsigned cp)
{
	HString *H = HString::Uni(cp);
	BSTR p, q = H->B;
...
CMarkdown::HSTR CMarkdown::_HSTR::Trim(const OLECHAR *pszTrimChars)
{
	HSTR H = this;
	BSTR B = H->B;
	if (StrTrimW(B, pszTrimChars) && SysReAllocStringLen(&B, B, lstrlenW(B)))
	{
		H = (HSTR)B;
	}
	return H;
...
: pvImage(NULL), cbImage(0), nByteOrder(0)
{
	...
	DWORD dwBOM = 0x3F3F3F3F; // all bytes which do not occur in a BOM
	memcpy(&dwBOM, pvImage, min<DWORD>(cbImage, 4));
	nByteOrder = GuessByteOrder(dwBOM);
	if (flags & Octets & nByteOrder)
	{
		LPVOID pCopy;
		switch (nByteOrder)
		{
		...
		case 2 + 1 + 8:
			...
			if (pCopy)
			{
				_swab(pcImage, (char *)pCopy, cbImage);
			}
			UnmapViewOfFile(pvImage);
			pvImage = pCopy;
			if (pvImage)
			{
			case 2 + 0:
		...
		case 4 + 2 + 8:
			...
			case 4 + 3 + 8:
				...
				if (pCopy)
				{
					converter.Convert(pchImage, cchImage, (PCH)pCopy, cbImage);
				}
				UnmapViewOfFile(pvImage);
				pvImage = pCopy;
			}
