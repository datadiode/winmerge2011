...
{
	DDX_Text<op>(IDC_COMPAREIMAGE_PATTERNS, m_sFilePatterns);
	return true;
}
...
LRESULT PropCompareImage::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		switch (wParam)
		{
		case MAKEWPARAM(IDC_COMPAREIMAGE_DEFAULTS, BN_CLICKED):
			OnDefaults();
			break;
		case MAKEWPARAM(IDC_COMPAREIMAGE_PATTERNS, CBN_DROPDOWN):
			WildcardDropList::OnDropDown(reinterpret_cast<HWND>(lParam), 6,
				COptionsMgr::GetDefault(OPT_CMP_IMG_FILEPATTERNS).c_str());
			break;
		case MAKEWPARAM(IDC_COMPAREIMAGE_PATTERNS, CBN_CLOSEUP):
