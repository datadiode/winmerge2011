...
String TempFile::Create(LPCTSTR prefix)
{
	...
	temp = env_GetTempFileName(temp.c_str(), pref.c_str());
	if (!temp.empty())
		m_path = temp;
 ...
 */
void CleanupWMtempfolder(const vector <int> &processIDs)
{
 ...
 */
BOOL WMrunning(const vector<int> &processIDs, int iPI)
{
	vector<int>::const_iterator iter = processIDs.begin();
 ...
 */
void ClearTempfolder(const String &pathName)
{
