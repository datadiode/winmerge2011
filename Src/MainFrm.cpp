...
HRESULT CMainFrame::ShowHTMLDialog(BSTR url, VARIANT *arguments, BSTR features, VARIANT *ret)
{
	...
	HWND const hWnd = bEnabled ? m_hWnd : ::GetActiveWindow();
	hr = MSHTML->ShowHTMLDialogEx(hWnd, spMoniker, flags, arguments, features, ret);
...
LRESULT CMainFrame::OnWndMsg<WM_INITMENUPOPUP>(WPARAM wParam, LPARAM lParam)
{
	...
	while (i)
	{
		...
		case ID_COLORSCHEME_FIRST:
			...
			while (TextDefinition const *const def =
				TextDefinition::GetTextType(mii.wID - ID_COLORSCHEME_FIRST))
			{
...
bool CMainFrame::ParseArgsAndDoOpen(const MergeCmdLineInfo &cmdInfo)
{
	...
	if (cmdInfo.m_Files.size() == 0) // if there are no input args, we can check the display file dialog flag
	{
		if (!cmdInfo.m_sRunScript.empty())
		{
			...
			CMyVariant ret;
			HRESULT hr = ShowHTMLDialog(const_cast<BSTR>(cmdInfo.m_sRunScript.c_str()), &CMyVariant(this), NULL, &ret);
			if (FAILED(hr))
			{
	...
	else
	{
		...
		if (bCompared && !cmdInfo.m_sRunScript.empty())
		{
			...
			{
				CMyVariant ret;
				HRESULT hr = ShowHTMLDialog(const_cast<BSTR>(cmdInfo.m_sRunScript.c_str()),
