...
static BOOL IsTexKeyword(LPCTSTR pszChars, int nLength)
{
	static LPCTSTR const s_apszTexKeywordList[] =
	{
		_T("above"),
		_T("acute"),
		_T("addtocounter"),
	...
	return xiskeyword<_tcsnicmp>(pszChars, nLength, s_apszTexKeywordList);
}
...
void TextDefinition::ParseLineTex(TextBlock::Cookie &cookie, LPCTSTR const pszChars, int const nLength, int I, TextBlock::Array &pBuf) const
{
...
TESTCASE
{
	...
	while (_fgetts(text, _countof(text), file))
	{
		...
		else if (_stscanf(text, _T(" static BOOL IsTexKeyword %c"), &c) == 1 && c == '(')
			pfnIsKeyword = IsTexKeyword;
