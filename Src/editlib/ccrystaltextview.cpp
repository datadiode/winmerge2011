...
CCrystalTextView::TextDefinition CCrystalTextView::m_SourceDefs[] =
{
	...
	SRC_ASP, _T ("ASP"), _T ("asp"), &ParseLineAsp, SRCOPT_AUTOINDENT|SRCOPT_BRACEANSI, /*2,*/ _T (""), _T (""), _T ("'"), (DWORD)-1,
...
void CCrystalTextView::ScrollToChar(int nNewOffsetChar, BOOL, BOOL bTrackScrollBar)
{
 ...
 */
void CCrystalTextView::ScrollToSubLine(int nNewTopSubLine, BOOL, BOOL bTrackScrollBar)
{
...
static void AppendStringAdv(String & str, int & curpos, LPCTSTR szadd)
{
	str += szadd;
...
static void AppendEscapeAdv(String & str, int & curpos, TCHAR c)
{
	String::size_type curlen = str.size();
	str.resize(curlen + 3);
	curpos += wsprintf(&str.front() + curlen, _T("\t%02X"), static_cast<int>(c));
}
...
{
	return m_pColors ? m_pColors->GetBold(nColorIndex & ~COLORINDEX_APPLYFORCE) : FALSE;
}
