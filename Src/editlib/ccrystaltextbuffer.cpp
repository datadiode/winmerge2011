...
, m_bAllowNewlinesInQuotes(false)
{
	...
	SetTextType(TextDefinition::SRC_PLAIN);
}
...
TextDefinition const *CCrystalTextBuffer::SetTextType(LPCTSTR pszExt)
{
	m_CurSourceDef = TextDefinition::m_SourceDefs;
	TextDefinition const *def = TextDefinition::GetTextType(pszExt);
	return SetTextType(def);
}

TextDefinition const *CCrystalTextBuffer::SetTextType(TextDefinition::TextType enuType)
{
	m_CurSourceDef = TextDefinition::m_SourceDefs;
	TextDefinition const *def = TextDefinition::GetTextType(enuType);
	return SetTextType(def);
}

TextDefinition const *CCrystalTextBuffer::SetTextType(TextDefinition const *def)
{
...
TextDefinition const *CCrystalTextBuffer::SetTextTypeByContent(LPCTSTR pszContent)
{
	Captures captures;
	if (FindStringHelper(pszContent, static_cast<UINT>(_tcslen(pszContent)),
		_T("^\\s*\\<\\?xml\\s+.+?\\?\\>\\s*$"), FIND_REGEXP, captures) >= 0)
	{
		return SetTextType(TextDefinition::SRC_XML);
	}
	return NULL;
}
...
void CCrystalTextBuffer::ParseLine(TextBlock::Cookie &cookie, int nLineIndex, TextBlock::Array &pBuf) const
{
	if (LPCTSTR const pszChars = GetLineChars(nLineIndex))
	{
		int const nLength = GetLineLength(nLineIndex);
		(m_CurSourceDef->*m_CurSourceDef->ParseLineX)(cookie, pszChars, nLength, -1, pBuf);
	}
}
